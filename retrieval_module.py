...
class RetrievalModule:
    def __init__(self, embedding_dim: int = 384):
        ...
        self.chunk_mapping = []  # To map index entries back to code chunks

    def embed_chunks(self, code_chunks):
        """
        Given a list of code chunks (each with a 'content' key), generate embeddings.
        """
        texts = [chunk["content"] for chunk in code_chunks]
        embeddings = self.model.encode(texts, convert_to_numpy=True)
        self.index.reset()  # Clear any existing data in the index
    ...
    def search(self, query: str, top_k: int = 3):
        """
        Given a query, return the top_k most similar code chunks.
        """
